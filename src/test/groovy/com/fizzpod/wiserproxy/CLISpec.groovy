package com.fizzpod.wiserproxy

import spock.lang.Specification

class CLISpec extends Specification {

    def "should parse default arguments when none are provided"() {
        when:
        def options = CLI.parse([] as String[])

        then:
        options != null
        options.port == 9080
        options.url == "wiser.local"
        options.c == 5
        !options.secret
    }

    def "should override cache ttl with short option -c"() {
        when:
        def options = CLI.parse(["-c", "15"] as String[])

        then:
        options.c == 15
    }

    def "should override cache ttl with long option --cache-ttl"() {
        when:
        def options = CLI.parse(["--cache-ttl", "30"] as String[])

        then:
        options.c == 30
    }

    def "should override port with short option -p"() {
        when:
        def options = CLI.parse(["-p", "1234"] as String[])

        then:
        options.port == 1234
    }

    def "should override port with long option --port"() {
        when:
        def options = CLI.parse(["--port", "5678"] as String[])

        then:
        options.port == 5678
    }

    def "should override url with short option -u"() {
        when:
        def options = CLI.parse(["-u", "http://hub1"] as String[])

        then:
        options.url == "http://hub1"
    }

    def "should override url with long option --url"() {
        when:
        def options = CLI.parse(["--url", "https://hub2"] as String[])

        then:
        options.url == "https://hub2"
    }

    def "should override secret with short option -s"() {
        when:
        def options = CLI.parse(["-s", "secret1"] as String[])

        then:
        options.secret == "secret1"
    }

    def "should override secret with long option --secret"() {
        when:
        def options = CLI.parse(["--secret", "secret2"] as String[])

        then:
        options.secret == "secret2"
    }

    def "should detect help flag"() {
        when:
        def options1 = CLI.parse(["-h"] as String[])
        def options2 = CLI.parse(["--help"] as String[])

        then:
        options1.h
        options2.h
    }
}
